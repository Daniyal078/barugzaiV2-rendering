...
function storageUrl(path?: string | null): string | undefined {
    ...
    return `${api_base_url}/storage/app/public/${path.replace(/^\//, '')}`
}
...
export default function CarsGrid({ cars }: CarsGridProps) {
    const [interestedCar, setInterestedCar] = useState<any | null>(null)
    const trackClick = useCallback((type: 'call' | 'whatsapp', carId: number, sellerId?: number) => {
        fetch(`${api_base_url}/api/v1/track/click`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ type, car_id: carId, seller_id: sellerId ?? null }),
        }).catch(console.error)
    }, [])
    return (
        <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {cars.map((car: any) => (
                    <CarCard
                        key={car.id}
                        car={car}
                        onInterested={(selectedCar) => setInterestedCar(selectedCar)}
                    />
                ))}
...
function InterestedModal({ car, onClose, onTrack }: InterestedModalProps) {
                ...
                <div className="flex flex-col items-center px-8 pt-10 pb-8 text-center">
                    ...
                    <div className="flex flex-col gap-3 w-full">
                        <a
                            ...
                            className="flex items-center justify-center gap-3 w-full py-3.5 rounded-xl bg-primary hover:bg-primary/90 text-white font-bold text-sm uppercase tracking-widest transition shadow-md"
                        >
                            <Phone className="w-4 h-4" /> Call Now
                        </a>
                        <Link href={`https://wa.me/+${waNumber?.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(
                            `I am interested in ${car.manufacturer?.title || ''}. Is it still available?`
                        )}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            onClick={() => onTrack('whatsapp', car.id, car.seller_id)}
                            className="flex items-center justify-center gap-3 w-full py-3.5 rounded-xl border text-[#25D366] border-[#25D366] hover:text-white hover:bg-[#1ebe5d]  font-bold text-sm uppercase tracking-widest transition shadow-md"
                        >
